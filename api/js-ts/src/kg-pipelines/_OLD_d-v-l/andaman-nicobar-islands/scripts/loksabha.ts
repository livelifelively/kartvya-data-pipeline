...
(async () => {
  ...
  // await upsert_Name_("election commission of india");
  // await upsert_Name_("ECI");

  // const graphQLClient = await createGraphQLClient();

  // const sourceId = await createNodeType("_Source_", graphQLClient, {
  //   name_id: "election-commission-of-india",
  //   names: [{ name: "election commission of india" }, { name: "ECI" }],
  //   source_description: "election commission of india",
  //   source_urls: ["https://results.eci.gov.in/"],
  // });

  const loksabhaConstituenciesList = [
    {
      names: ["Andaman and Nicobar Islands"],
      wikipedia_page: "https://en.wikipedia.org/wiki/Andaman_and_Nicobar_Islands_Lok_Sabha_constituency",
    },
  ];
